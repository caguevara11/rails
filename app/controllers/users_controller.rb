class UsersController < ApplicationController
  ...
  def update
    if @user.update(user_params)
      redirect_to user_url(@user), notice: t("application.was_updated", model: t("activerecord.modules.users.one"))
    else
      render :new, status: :unprocessable_entity
    end
  end
  ...
  def destroy
    @user.destroy
    redirect_to users_url, notice: t("application.was_destroyed", model: t("activerecord.modules.users.one"))
  end
